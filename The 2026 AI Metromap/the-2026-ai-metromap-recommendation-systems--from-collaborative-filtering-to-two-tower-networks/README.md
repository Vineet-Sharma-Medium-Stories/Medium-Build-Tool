# The 2026 Ai Metromap Recommendation Systems  From Collaborative Filtering To Two Tower Networks

Generated: 2026-04-01 21:08:58

## Summary
- Tables: 0 (0 successful)
- Diagrams: 2 (2 successful)
- PNG Images: 3
- Copied Images: 1

## Combined Document
- [the-2026-ai-metromap-recommendation-systems--from-collaborative-filtering-to-two-tower-networks.md](the-2026-ai-metromap-recommendation-systems--from-collaborative-filtering-to-two-tower-networks.md) - Single file with all images

---
### Rendering Method
- GitHub-style markdown tables with proper formatting (bold, italic, code, emojis)
- Mermaid diagrams rendered with mermaid.js
- Titles extracted from bold text (preferred) or headings
- All content rendered with Playwright (headless Chromium) for high quality
